#!/bin/bash
# =============================================
# SMARTCHOICE EVENT MANAGER v5.0 - FINAL
# Ubuntu 22.04/24.04 + PHP 8.2/8.3 + SQLite
# Menus organizados + Português correto + LED Calc + Escala Técnicos
# =============================================
set -e
clear
cat << "BANNER"
...
║  Instalação Completa Final                   ║
╚══════════════════════════════════════════════╝
BANNER

# Detetar Ubuntu e PHP
...
# =============================================
# 2. LARAVEL + FILAMENT + DEPENDÊNCIAS
# =============================================
...
Schema::create('orcamentos',function(\$t){\$t->id();\$t->string('numero')->unique();\$t->string('cliente_nome');\$t->string('cliente_email')->nullable();\$t->string('cliente_telefone')->nullable();\$t->string('evento_nome')->nullable();\$t->string('evento_local')->nullable();\$t->date('data_inicio');\$t->date('data_fim');\$t->string('estado')->default('orçamentação');\$t->decimal('valor_total',10,2)->default(0);\$t->text('notas')->nullable();\$t->timestamps();});
...
Schema::create('guia_transportes',function(\$t){\$t->id();\$t->string('numero')->unique();\$t->foreignId('orcamento_id')->nullable()->constrained('orcamentos')->onDelete('set null');\$t->string('tipo')->default('carga');\$t->string('estado')->default('pendente');\$t->string('responsavel')->nullable();\$t->text('observações')->nullable();\$t->timestamps();});
...
Schema::create('reparações',function(\$t){\$t->id();\$t->foreignId('equipamento_id')->constrained('equipamentos')->onDelete('cascade');\$t->text('descrição_avaria');\$t->string('estado')->default('reportado');\$t->string('técnico')->nullable();\$t->decimal('custo_reparação',10,2)->nullable();\$t->date('data_entrada')->nullable();\$t->date('data_saída')->nullable();\$t->text('notas_técnicas')->nullable();\$t->timestamps();});
Schema::create('colaboradores',function(\$t){\$t->id();\$t->string('nome');\$t->string('morada')->nullable();\$t->string('bi_passaporte')->nullable();\$t->string('função')->nullable();\$t->text('competências')->nullable();\$t->integer('idade')->nullable();\$t->text('epis')->nullable();\$t->text('dados_adicionais')->nullable();\$t->timestamps();});
Schema::create('colaborador_equipamento',function(\$t){\$t->id();\$t->foreignId('colaborador_id')->constrained('colaboradores')->onDelete('cascade');\$t->foreignId('equipamento_id')->constrained('equipamentos')->onDelete('cascade');\$t->integer('quantidade')->default(1);\$t->date('data_atribuição')->nullable();\$t->date('data_devolução')->nullable();\$t->text('notas')->nullable();\$t->string('tipo')->default('Equipamento');\$t->foreignId('numero_serie_id')->nullable()->constrained('numeros_serie')->onDelete('set null');\$t->timestamps();});
Schema::create('entidades',function(\$t){\$t->id();\$t->string('nome');\$t->string('designação_comercial')->nullable();\$t->string('tipo_entidade')->default('Cliente');\$t->string('nif')->nullable();\$t->string('país')->default('Portugal');\$t->string('email')->nullable();\$t->string('telefone')->nullable();\$t->text('morada')->nullable();\$t->text('notas')->nullable();\$t->boolean('ativo')->default(true);\$t->timestamps();});
Schema::create('funções',function(\$t){\$t->id();\$t->string('nome');\$t->foreignId('departamento_id')->nullable()->constrained('categorias')->onDelete('set null');\$t->text('descrição')->nullable();\$t->boolean('ativo')->default(true);\$t->timestamps();});
Schema::create('logs_stock',function(\$t){\$t->id();\$t->foreignId('orcamento_id')->constrained('orcamentos')->onDelete('cascade');\$t->foreignId('equipamento_id')->constrained('equipamentos')->onDelete('cascade');\$t->integer('quantidade_antes');\$t->integer('quantidade_depois');\$t->string('ação');\$t->timestamps();});
Schema::create('escalas_técnicos',function(\$t){\$t->id();\$t->foreignId('colaborador_id')->constrained('colaboradores')->onDelete('cascade');\$t->foreignId('orcamento_id')->nullable()->constrained('orcamentos')->onDelete('cascade');\$t->date('data_início');\$t->date('data_fim');\$t->time('hora_entrada')->nullable();\$t->time('hora_saída')->nullable();\$t->string('função')->nullable();\$t->text('notas')->nullable();\$t->timestamps();});
echo 'OK';
" --quiet
echo "✅"

# =============================================
# 4. MODELOS + OBSERVER + ADMIN
# =============================================
echo "[4/8] Modelos + Observer + Admin"
...
for m in "Categoria:categorias:nome,parent_id,descrição,ordem" "Equipamento:equipamentos:nome,marca,modelo,categoria_id,estado,quantidade,preço_aluguer_dia,preço_custo,armazem,notas" "NumeroSerie:numeros_serie:equipamento_id,numero_serie,qr_code,estado" "Orcamento:orcamentos:número,cliente_nome,cliente_email,cliente_telefone,evento_nome,evento_local,data_início,data_fim,estado,valor_total,notas" "OrcamentoItem:orcamento_itens:orcamento_id,equipamento_id,quantidade,preço_unitario,dias,subtotal,subaluguer,fornecedor,custo_subaluguer" "GuiaTransporte:guia_transportes:número,orcamento_id,tipo,estado,responsavel,observações" "GuiaItem:guia_itens:guia_transporte_id,equipamento_id,quantidade,notas" "Reparacao:reparações:equipamento_id,descrição_avaria,estado,técnico,custo_reparação,data_entrada,data_saída,notas_técnicas" "Colaborador:colaboradores:nome,morada,bi_passaporte,função,competências,idade,epis,dados_adicionais" "Entidade:entidades:nome,designação_comercial,tipo_entidade,nif,país,email,telefone,morada,notas,ativo" "Funcao:funções:nome,departamento_id,descrição,ativo" "EscalaTecnico:escalas_técnicos:colaborador_id,orcamento_id,data_início,data_fim,hora_entrada,hora_saída,função,notas"; do
...
# Relações especiais (resumidas)
cat > app/Models/Categoria.php << 'EOF'
<?php namespace App\Models;use Illuminate\Database\Eloquent\Model;class Categoria extends Model{protected $table='categorias';protected $fillable=['nome','parent_id','descrição','ordem'];public function parent(){return $this->belongsTo(Categoria::class,'parent_id');}public function children(){return $this->hasMany(Categoria::class,'parent_id');}public function equipamentos(){return $this->hasMany(Equipamento::class,'categoria_id');}}
EOF
cat > app/Models/Orcamento.php << 'EOF'
<?php namespace App\Models;use Illuminate\Database\Eloquent\Model;class Orcamento extends Model{protected $table='orcamentos';protected $fillable=['número','cliente_nome','cliente_email','cliente_telefone','evento_nome','evento_local','data_início','data_fim','estado','valor_total','notas'];public function itens(){return $this->hasMany(OrcamentoItem::class);}public function guias(){return $this->hasMany(GuiaTransporte::class);}public function escalas(){return $this->hasMany(EscalaTecnico::class);}}
EOF
cat > app/Models/Colaborador.php << 'EOF'
<?php namespace App\Models;use Illuminate\Database\Eloquent\Model;class Colaborador extends Model{protected $table='colaboradores';protected $fillable=['nome','morada','bi_passaporte','função','competências','idade','epis','dados_adicionais'];public function equipamentos(){return $this->belongsToMany(Equipamento::class,'colaborador_equipamento')->withPivot('quantidade','data_atribuição','data_devolução','notas','numero_serie_id','tipo')->withTimestamps();}public function escalas(){return $this->hasMany(EscalaTecnico::class);}}
EOF
cat > app/Models/EscalaTecnico.php << 'EOF'
<?php namespace App\Models;use Illuminate\Database\Eloquent\Model;class EscalaTecnico extends Model{protected $table='escalas_técnicos';protected $fillable=['colaborador_id','orcamento_id','data_início','data_fim','hora_entrada','hora_saída','função','notas'];public function colaborador(){return $this->belongsTo(Colaborador::class);}public function orcamento(){return $this->belongsTo(Orcamento::class);}}
EOF
...
# =============================================
# 5. RESOURCES + DASHBOARD + WIDGETS + ESCALA
# =============================================
echo "[5/8] Resources + Dashboard + Escala"
for r in Categoria Equipamento Orcamento GuiaTransporte Reparacao Colaborador Entidade Funcao User; do php artisan make:filament-resource $r --generate --no-interaction --quiet; done

# Corrigir nomes nos Resources
sed -i "s/'navigationLabel' => 'Categorias'/'navigationLabel' => 'Departamentos'/" app/Filament/Resources/CategoriaResource.php
sed -i "s/'pluralModelLabel' => 'Categorias'/'pluralModelLabel' => 'Departamentos'/" app/Filament/Resources/CategoriaResource.php
sed -i "s/'modelLabel' => 'Categoria'/'modelLabel' => 'Departamento'/" app/Filament/Resources/CategoriaResource.php
sed -i "s/'navigationLabel' => 'Colaboradors'/'navigationLabel' => 'Colaboradores'/" app/Filament/Resources/ColaboradorResource.php
sed -i "s/'pluralModelLabel' => 'Colaboradors'/'pluralModelLabel' => 'Colaboradores'/" app/Filament/Resources/ColaboradorResource.php
sed -i "s/'pluralModelLabel' => 'Funcaos'/'pluralModelLabel' => 'Funções'/" app/Filament/Resources/FuncaoResource.php
sed -i "s/'navigationLabel' => 'Funcaos'/'navigationLabel' => 'Funções'/" app/Filament/Resources/FuncaoResource.php
sed -i "s/'pluralModelLabel' => 'Reparacaos'/'pluralModelLabel' => 'Reparações'/" app/Filament/Resources/ReparacaoResource.php
sed -i "s/'navigationLabel' => 'Reparacaos'/'navigationLabel' => 'Reparações'/" app/Filament/Resources/ReparacaoResource.php
sed -i "s/'navigationLabel' => 'Guia Transportes'/'navigationLabel' => 'Guias de Transporte'/" app/Filament/Resources/GuiaTransporteResource.php
sed -i "s/'pluralModelLabel' => 'Guia Transportes'/'pluralModelLabel' => 'Guias de Transporte'/" app/Filament/Resources/GuiaTransporteResource.php
sed -i "s/'navigationLabel' => 'Users'/'navigationLabel' => 'Utilizadores'/" app/Filament/Resources/UserResource.php
sed -i "s/'pluralModelLabel' => 'Users'/'pluralModelLabel' => 'Utilizadores'/" app/Filament/Resources/UserResource.php

# Grupos de navegação
for r in Categoria Colaborador Funcao User; do
  sed -i "/protected static ?string \$navigationIcon = null;/a\    protected static ?string \$navigationGroup = 'Administração';" app/Filament/Resources/${r}Resource.php
done
for r in Equipamento GuiaTransporte Reparacao; do
  sed -i "/protected static ?string \$navigationIcon = null;/a\    protected static ?string \$navigationGroup = 'Logística';" app/Filament/Resources/${r}Resource.php
done
for r in Orcamento Entidade; do
  sed -i "/protected static ?string \$navigationIcon = null;/a\    protected static ?string \$navigationGroup = 'Comercial';" app/Filament/Resources/${r}Resource.php
done
...
<?php namespace App\Filament\Pages;use App\Filament\Widgets\StatsEquipamentos;use App\Filament\Widgets\AlertasConflitos;use App\Filament\Widgets\CalendarioEventos;use App\Filament\Widgets\CalendarioMensal;use Filament\Pages\Dashboard as BaseDashboard;class Dashboard extends BaseDashboard{protected static ?string $title='Dashboard';protected static ?string $navigationLabel='Dashboard';public function getWidgets():array{return[StatsEquipamentos::class,AlertasConflitos::class,CalendarioEventos::class,CalendarioMensal::class];}public function getColumns():int|string|array{return 1;}}
EOF

cat > app/Filament/Widgets/StatsEquipamentos.php << 'EOF'
<?php namespace App\Filament\Widgets;use App\Models\Equipamento;use App\Models\Orcamento;use Filament\Widgets\StatsOverviewWidget as BaseWidget;use Filament\Widgets\StatsOverviewWidget\Stat;class StatsEquipamentos extends BaseWidget{protected function getStats():array{$d=Equipamento::where('estado','disponivel')->sum('quantidade');$t=Equipamento::sum('quantidade');$m=Equipamento::where('estado','manutencao')->sum('quantidade');$c=Orcamento::where('estado','confirmado')->count();$o=Orcamento::where('estado','orçamentação')->count();$dr=Orcamento::where('estado','draft')->count();$ca=Orcamento::where('estado','cancelado')->count();return[Stat::make('Stock Disponível',$d)->description('Total: '.$t)->color('success'),Stat::make('Manutenção',$m)->description(Equipamento::where('estado','manutencao')->count().' equip.')->color($m>0?'danger':'success'),Stat::make('Orçamentos',$c+$o+$dr+$ca)->description(($c>0?'🟢'.$c.' ':'').($o>0?'🟡'.$o.' ':'').($dr>0?'🔵'.$dr.' ':'').($ca>0?'🔴'.$ca:''))->color('warning')];}}
EOF

cat > app/Filament/Widgets/AlertasConflitos.php << 'EOF'
<?php namespace App\Filament\Widgets;use App\Models\Orcamento;use Filament\Widgets\Widget;use Carbon\Carbon;class AlertasConflitos extends Widget{protected static string $view='filament.widgets.alertas-conflitos';protected int|string|array $columnSpan='full';public function getConflitos(){$c=[];$os=Orcamento::where('estado','confirmado')->where('data_fim','>=',Carbon::now())->with('itens.equipamento')->get();foreach($os as $o1){foreach($os as $o2){if($o1->id>=$o2->id)continue;$i1=Carbon::parse($o1->data_início);$f1=Carbon::parse($o1->data_fim);$i2=Carbon::parse($o2->data_início);$f2=Carbon::parse($o2->data_fim);if($i1->lte($f2)&&$f1->gte($i2)){$e1=$o1->itens->pluck('equipamento_id')->toArray();$e2=$o2->itens->pluck('equipamento_id')->toArray();$com=array_intersect($e1,$e2);if(!empty($com))$c[]=['o1'=>$o1->número.' - '.$o1->cliente_nome,'o2'=>$o2->número.' - '.$o2->cliente_nome,'p'=>$i1->format('d/m').'-'.$f1->format('d/m').' ↔ '.$i2->format('d/m').'-'.$f2->format('d/m'),'e'=>count($com)];}}}return $c;}}
EOF

cat > app/Filament/Widgets/CalendarioEventos.php << 'EOF'
<?php namespace App\Filament\Widgets;use App\Models\Orcamento;use Filament\Widgets\Widget;use Carbon\Carbon;class CalendarioEventos extends Widget{protected static string $view='filament.widgets.calendario-eventos';protected int|string|array $columnSpan='full';public function getOrcamentos(){$is=Carbon::now('Europe/Lisbon')->startOfWeek(Carbon::MONDAY)->startOfDay();$fs=Carbon::now('Europe/Lisbon')->endOfWeek(Carbon::SUNDAY)->endOfDay();return Orcamento::with(['itens.equipamento'])->whereIn('estado',['draft','confirmado','orçamentação'])->where(function($q)use($is,$fs){$q->whereBetween('data_início',[$is,$fs])->orWhereBetween('data_fim',[$is,$fs])->orWhere(function($q2)use($is,$fs){$q2->where('data_início','<=',$is)->where('data_fim','>=',$fs);});})->orderBy('data_início')->get()->map(function($o)use($is){$in=Carbon::parse($o->data_início)->startOfDay();$fi=Carbon::parse($o->data_fim)->startOfDay();$ci=max(1,min(7,$is->diffInDays($in,false)+1));$cf=max(1,min(7,$is->diffInDays($fi,false)+1));$d=max(1,$cf-$ci+1);$cor=match($o->estado){'confirmado'=>'#10B981','orçamentação'=>'#F59E0B','draft'=>'#3B82F6',default=>'#6B7280'};return(object)['id'=>$o->id,'numero'=>$o->número,'cliente'=>$o->cliente_nome,'evento'=>$o->evento_nome,'local'=>$o->evento_local,'inicio'=>$o->data_início,'fim'=>$o->data_fim,'coluna_inicio'=>$ci,'duracao'=>$d,'cor'=>$cor,'estado'=>$o->estado,'total_equipamentos'=>$o->itens->sum('quantidade'),'valor_total'=>$o->valor_total];});}public function getDiaAtual():int{return Carbon::now('Europe/Lisbon')->dayOfWeekIso;}public function getSemanaAtual():array{$d=[];$i=Carbon::now('Europe/Lisbon')->startOfWeek(Carbon::MONDAY)->startOfDay();Carbon::setLocale('pt');for($x=0;$x<7;$x++){$dd=$i->copy()->addDays($x);$d[]=['nome'=>$dd->translatedFormat('D'),'dia'=>$dd->format('d'),'mes'=>$dd->format('M'),'data'=>$dd->format('Y-m-d'),'hoje'=>$dd->isToday()];}return $d;}}
EOF

cat > app/Filament/Widgets/CalendarioMensal.php << 'EOF'
<?php namespace App\Filament\Widgets;use App\Models\Orcamento;use Filament\Widgets\Widget;use Carbon\Carbon;class CalendarioMensal extends Widget{protected static string $view='filament.widgets.calendario-mensal';protected int|string|array $columnSpan='full';public $mesAtual;public $anoAtual;public function mount(){$this->mesAtual=Carbon::now()->month;$this->anoAtual=Carbon::now()->year;}public function mesAnterior(){if($this->mesAtual==1){$this->mesAtual=12;$this->anoAtual--;}else{$this->mesAtual--;}}public function mesSeguinte(){if($this->mesAtual==12){$this->mesAtual=1;$this->anoAtual++;}else{$this->mesAtual++;}}public function getDiasDoMes(){$im=Carbon::create($this->anoAtual,$this->mesAtual,1);$fm=$im->copy()->endOfMonth();$dias=[];$da=$im->copy()->startOfWeek(Carbon::MONDAY);for($i=0;$i<42;$i++){$dias[]=['data'=>$da->format('Y-m-d'),'dia'=>$da->day,'mes_atual'=>$da->month==$this->mesAtual,'hoje'=>$da->isToday(),'fim_semana'=>$da->isWeekend(),'eventos'=>[]];$da->addDay();}$evs=Orcamento::with('itens.equipamento')->whereIn('estado',['draft','orçamentação','confirmado'])->where(function($q)use($im,$fm){$q->whereBetween('data_início',[$im,$fm])->orWhereBetween('data_fim',[$im,$fm])->orWhere(function($q2)use($im,$fm){$q2->where('data_início','<=',$im)->where('data_fim','>=',$fm);});})->orderBy('data_início')->get();foreach($dias as &$dia){foreach($evs as $ev){$ini=Carbon::parse($ev->data_início);$fim=Carbon::parse($ev->data_fim);if(Carbon::parse($dia['data'])->between($ini,$fim)){$cor=match($ev->estado){'confirmado'=>'#10B981','orçamentação'=>'#F59E0B','draft'=>'#3B82F6',default=>'#6B7280'};$dia['eventos'][]=['id'=>$ev->id,'numero'=>$ev->número,'cliente'=>$ev->cliente_nome,'evento'=>$ev->evento_nome,'local'=>$ev->evento_local,'estado'=>$ev->estado,'cor'=>$cor,'inicio'=>$ev->data_início,'fim'=>$ev->data_fim,'total_equipamentos'=>$ev->itens->sum('quantidade')];}}}return $dias;}public function getNomeMes():string{$meses=['','Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];return $meses[$this->mesAtual].' '.$this->anoAtual;}}
EOF

# Escala Técnicos (versão formatada)
...
class EscalaTecnicos extends Page
{
    ...
    public function carregar(){$i=Carbon::parse($this->semanaInicio);$f=$i->copy()->addDays(6);$this->tecnicos=Colaborador::orderBy('nome')->get()->toArray();$this->eventos=Orcamento::whereIn('estado',['confirmado','orçamentação','draft'])->where(function($q)use($i,$f){$q->whereBetween('data_início',[$i,$f])->orWhereBetween('data_fim',[$i,$f])->orWhere(function($q2)use($i,$f){$q2->where('data_início','<=',$i)->where('data_fim','>=',$f);});})->orderBy('data_início')->get()->toArray();$es=EscalaTecnico::with('colaborador','orcamento')->where(function($q)use($i,$f){$q->whereBetween('data_início',[$i,$f])->orWhereBetween('data_fim',[$i,$f]);})->get();$this->escalasData=[];foreach($es as $e){$cid=$e->colaborador_id;if(!isset($this->escalasData[$cid]))$this->escalasData[$cid]=[];$this->escalasData[$cid][]=['id'=>$e->id,'colaborador_id'=>$e->colaborador_id,'orcamento_id'=>$e->orcamento_id,'data_inicio'=>$e->data_início,'data_fim'=>$e->data_fim,'orcamento_numero'=>$e->orcamento->número??'','orcamento_cliente'=>$e->orcamento->cliente_nome??''];}return;}
    ...
    public function alocarTecnico($cid,$oid){$c=Colaborador::find($cid);$o=Orcamento::find($oid);if(!$c||!$o)return;if(EscalaTecnico::where('colaborador_id',$cid)->where(function($q)use($o){$q->whereBetween('data_início',[$o->data_início,$o->data_fim])->orWhereBetween('data_fim',[$o->data_início,$o->data_fim]);})->exists()){Notification::make()->title('Conflito')->body($c->nome.' já está alocado neste período.')->warning()->send();return;}EscalaTecnico::create(['colaborador_id'=>$cid,'orcamento_id'=>$oid,'data_início'=>$o->data_início,'data_fim'=>$o->data_fim,'função'=>$c->função]);$this->mostrarModal=false;Notification::make()->title('Alocado')->body($c->nome.' → '.$o->número)->success()->send();$this->carregar();}
...
# =============================================
# 8. PERMISSÕES + FINALIZAR
# =============================================
echo "[8/8] Permissões + Finalizar"
